...
void Question::setSubject(const string &subject)
{
    this->subject = subject;
}
...
void Question::setTitle(const string &title) { 
    this->title = title; 
}
...
void Question::setOptionA(const string &option) {
     this -> option_a = option;
     }
...
Question Question::deserialize(const string &line)
{
    ...
    q.setOptionD(token);
    getline(ss, token, '|');
    q.setAnswerKey(token[0]);
    getline(ss, token, '|');
    q.setEvaluationId(stoi(token));
    return q;
}
